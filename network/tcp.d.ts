...
declare module "tcp" {
   ...
   * @param dev {string} The network interface you want to bind. default: not bind
   */
   ...
   * Returns: {Object} Tcp object.
   *
   * @param sockFd {number} Socket file descriptor, MUST tcp socket.
   */
  ...
  class Tcp {
    ...
    send(buffer: Buffer[], timeout?: number): number;
    ...
    sendto(remoteAddr: SockAddr, buffer: Buffer[], timeout?: number): number;
