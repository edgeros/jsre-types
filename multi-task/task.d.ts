...
declare module "task" {
  ...
  class Task {
     ...
     */
    static list(exceptMe?: boolean): Array<number>;
     ...
     */
    cleanup(callback: Function, ...args: any): void;
    ...
    send(buff: Buffer, offset?: number, length?: number, addition?: Object): boolean;

    static on(event: "message", handler: (...args: any) => void);
    static on(event: "uncaughtException", handler: (error: Error) => void);
    static on(event: "unhandledRejection", handler: (reason: any, promise: any) => void);
  }
